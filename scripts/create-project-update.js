...
async function createProjectUpdate() {
  try {
    ...
    const projectResponse = await fetch(`http://localhost:1338/api/projects?filters[Slug][$eq]=${projectSlug}&fields[0]=id&fields[1]=documentId&fields[2]=Title`, {
      headers: {
    ...
    const createResponse = await fetch('http://localhost:1338/api/project-updates', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
