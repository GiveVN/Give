...
const rateLimit = (maxRequests: number, windowMs: number) => {
  return async (ctx: any, next: () => Promise<void>) => {
    const identifier = `${ctx.state.user?.id || ctx.ip}-${ctx.request.path}`
    const now = Date.now()
...
export default ({ strapi }: { strapi: Core.Strapi }) => ({
   ...
   */
  uploadAvatar: [
    rateLimit(5, 24 * 60 * 60 * 1000), // 5 requests per day
    async (ctx: any) => {
      const userId = ctx.state.user?.id
      ...
      if (!allowedTypes.includes(avatarFile.mimetype)) {
        return ctx.badRequest('Invalid file type. Only JPEG, PNG, and WebP are allowed.')
      }
