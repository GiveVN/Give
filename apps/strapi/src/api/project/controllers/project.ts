  ...
  ({ strapi }) => ({
    ...
    async getProgress(ctx) {
      ...
      try {
        ...
        const updatedMilestones =
          project.GoalMilestones?.map((milestone) => {
            if (
              !milestone.IsReached &&
              project.CurrentFunding >= milestone.TargetAmount
            ) {
              return {
                ...milestone,
                IsReached: true,
        ...
        const milestonesChanged = updatedMilestones.some(
          (milestone, index) =>
        ...
        const recentDonations =
          project.Donations?.filter((d) => new Date(d.createdAt) > oneDayAgo) ||
          []

        const recentDonationsTotal = recentDonations.reduce(
        ...
        return ctx.send({
          ...
          milestones: updatedMilestones.sort(
    ...
    async updateFunding(ctx) {
      ...
      try {
        ...
        const newFunding =
          parseFloat(project.CurrentFunding?.toString() || "0") +
          parseFloat(amount)
    ...
    async getStatistics(ctx) {
      ...
      try {
        ...
        const averageDonation =
          totalDonations > 0 ? project.CurrentFunding / totalDonations : 0

        // Get top backers (non-anonymous)
        const topBackers =
          project.Donations?.filter((d) => !d.IsAnonymous && d.Giver)
