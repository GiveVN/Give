...
import { commentReplyTemplate } from '../../../extensions/email/templates/comment-reply';
  ...
  async create(ctx) {
    ...
    if (commentData.Parent) {
      try {
        ...
        if (parentComment && parentComment.Author && parentComment.Author.email) {
          ...
          const emailNotificationsEnabled = author.emailNotifications !== false;
          ...
          if (emailNotificationsEnabled) {
            ...
            const templateData = {
              ...
              reply: response.data,
              replyAuthor: response.data.Author ? response.data.Author.username : 'Anonymous',
              project: project,
            ...
            console.log(`Reply notification email sent to ${parentComment.Author.email}`);
          } else {
            console.log(`User ${parentComment.Author.email} has disabled email notifications`);
          }
        }
      } catch (error) {
