...
export async function GET(
  ...
  try {
    ...
    const hourlyData = new Array(24)
      .fill(0)
      .map((_, hour) => ({ hour, amount: 0, count: 0 }))
    donations.forEach((d) => {
    ...
    sizeRanges.forEach((range) => {
      range.percentage =
        totalBackers > 0 ? Math.round((range.count / totalBackers) * 100) : 0
    })
    ...
    const geographic = [
      {
        ...
        count: Math.floor(totalBackers * 0.4),
        amount: Math.floor(totalRaised * 0.4),
      },
      {
        country: "Canada",
        count: Math.floor(totalBackers * 0.2),
        amount: Math.floor(totalRaised * 0.2),
      },
      {
        country: "United Kingdom",
        count: Math.floor(totalBackers * 0.15),
        amount: Math.floor(totalRaised * 0.15),
      },
      {
        country: "Germany",
        count: Math.floor(totalBackers * 0.1),
        amount: Math.floor(totalRaised * 0.1),
      },
      {
        country: "Australia",
        count: Math.floor(totalBackers * 0.08),
        amount: Math.floor(totalRaised * 0.08),
      },
      {
        country: "France",
        count: Math.floor(totalBackers * 0.07),
        amount: Math.floor(totalRaised * 0.07),
      },
    ]
    ...
    const devices = [
      {
        type: "Desktop",
        count: Math.floor(totalBackers * 0.6),
        percentage: 60,
      },
      {
        type: "Mobile",
        count: Math.floor(totalBackers * 0.35),
        percentage: 35,
      },
    ...
    const milestones = (project.data.GoalMilestones || []).map((m) => ({
      ...
      reached: m.IsReached,
      date: m.ReachedAt,
      daysToReach: m.ReachedAt
        ? Math.ceil(
            (new Date(m.ReachedAt).getTime() - projectStartDate.getTime()) /
              (1000 * 60 * 60 * 24)
          )
        : undefined,
    }))
    ...
    const sources = [
      {
        source: "Direct",
        visits: Math.floor(totalBackers * 2.5),
        conversions: Math.floor(totalBackers * 0.4),
        amount: Math.floor(totalRaised * 0.4),
      },
      {
        source: "Social Media",
        visits: Math.floor(totalBackers * 3),
        conversions: Math.floor(totalBackers * 0.3),
        amount: Math.floor(totalRaised * 0.3),
      },
      {
        source: "Search",
        visits: Math.floor(totalBackers * 2),
        conversions: Math.floor(totalBackers * 0.2),
        amount: Math.floor(totalRaised * 0.2),
      },
      {
        source: "Email",
        visits: Math.floor(totalBackers * 1.5),
        conversions: Math.floor(totalBackers * 0.1),
        amount: Math.floor(totalRaised * 0.1),
      },
    ]
